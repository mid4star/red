      ...
      try {
        ...
        if (json.success && json.data && json.data.length > 0) {
          const mapped = json.data.map((r: any) => ({
            ...
            img: r.imageUrl || '/red_sea_hero_aerial_1774790601114.png',
            coords: r.coords || '26.5000° N, 34.8000° E',
            speciesCount: r.speciesCount || 500,
            healthIndex: r.healthIndex || 9.5,
            status: r.status,
            statusAr: r.statusAr || (r.status === 'OPEN' ? (isAr ? 'مفتوحة' : 'Open') : (isAr ? 'مقيدة' : 'Restricted')),
            location: r.location,
        ...
        console.error('Error loading reserves from DB:', e);
        setReserves(RESERVES);
      } finally {
        setLoading(false);
      }
                 ...
                 >
                    ...
                    <div className="w-full lg:w-3/5 relative group">
                       ...
                       <div className="aspect-[16/10] rounded-[3rem] overflow-hidden border border-white/10 shadow-2xl relative">
                          ...
                          <div className="absolute bottom-4 right-4 left-4 sm:bottom-8 sm:right-8 sm:left-8 flex justify-between items-end">
                             <div className="space-y-1">
                                <p className="text-[9px] sm:text-[10px] font-black text-teal-400 uppercase tracking-widest italic">{isAr ? 'الإحداثيات المكانية' : 'Spatial Coordinates'}</p>
                                <p className="text-xs sm:text-sm font-black italic tracking-tighter text-white uppercase">{item.coords}</p>
                             </div>
                    ...
                    <div className="w-full lg:w-2/5 space-y-6 lg:space-y-10">
                       ...
                       <div className="pt-6">
                          <Link href={`/${lang}/reserves/${item.id}`} className="no-underline">
