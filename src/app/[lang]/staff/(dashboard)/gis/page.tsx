...
import { Map as MapIcon, BarChart3, LayoutDashboard, Settings, FileText, Lock } from 'lucide-react';
...
import GISPrivateMaps from './components/GISPrivateMaps';
...
export default function GISPage({ params: { lang } }: { params: { lang: string } }) {
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-6" dir={isArabic ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-wrap items-center justify-between bg-th-surface2 p-4 md:p-6 rounded-2xl border border-th-border shadow-sm gap-4 transition-colors duration-300">
        ...
        {/* Tab Navigation */}
        <div className="flex flex-wrap p-1 bg-th-surface border border-th-border rounded-xl gap-1">
          <button 
          ...
          <button 
            onClick={() => setActiveTab('private-maps')}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'private-maps' ? 'bg-rose-500 text-white shadow-md' : 'text-th-muted hover:text-th-text hover:bg-th-surface2'}`}
          >
            <Lock size={16} />
            {isArabic ? 'الخرائط الخاصة' : 'Private Maps'}
          </button>
          <button 
      ...
      <div className="w-full">
        ...
        {activeTab === 'private-maps' && <GISPrivateMaps isArabic={isArabic} />}
