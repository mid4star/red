...
export default function DataGrid({ columns, data, onRowClick, onEdit, onDelete, lang, emptyMessage, filterContent }: DataGridProps) {
  const isAr = lang === 'ar';
  ...
  return (
    <div className="w-full bg-th-surface rounded-2xl border border-th-border flex flex-col h-full overflow-hidden shadow-sm transition-all duration-300">
      ...
      {filterContent && (
        <div className="p-4 md:px-5 md:py-3 border-b border-th-border bg-th-surface/50">
          {filterContent}
        </div>
      )}
